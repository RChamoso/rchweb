...
function menuPrincipal() {
  ...
  for (var i in listaMenu) {
    ...
    var subMenu = elementoNuevo('ul');
      subMenu.setAttribute('id', listaMenu[i].subMenu);
      subMenu.setAttribute('class', 'subMenu');
      var opcionesSubMenu = listaMenu[i].contenido;
      for (var j in opcionesSubMenu) {
        var opcionSubMenu = elementoNuevo('li');
          var tipoFuncion = opcionesSubMenu[j].subTitulo;
          var funcionSubMenu = '';
          if (tipoFuncion !=  '') {
            var dato = [];
            funcionSubMenu = opcionesSubMenu[j].funcion + '("' + opcionesSubMenu[j].subTitulo + '", "' + opcionesSubMenu[j].enlace + '", "' + opcionesSubMenu[j].titulo + '");';
          } else if (opcionesSubMenu[j].funcion ==  'mostrarSubMenu') {
            funcionSubMenu = opcionesSubMenu[j].funcion + '("' + opcionesSubMenu[j].subTitulo + '", "' + i + '", "' + j + '", "' + opcionesSubMenu[j].titulo + '");';
          } else if (opcionesSubMenu[j].funcion ==  'fuentes()') {
            funcionSubMenu = opcionesSubMenu[j].funcion;
          } else {
            funcionSubMenu = opcionesSubMenu[j].funcion + '("' + opcionesSubMenu[j].enlace + '", false);';
          }
          if (opcionesSubMenu[j].funcion == 'nuevaVentana') {
            opcionSubMenu.setAttribute('class', 'nuevaVentana');
          }
          opcionSubMenu.setAttribute('onclick',funcionSubMenu);
          opcionSubMenu.textContent = opcionesSubMenu[j].titulo;
        subMenu.appendChild(opcionSubMenu);
      }
